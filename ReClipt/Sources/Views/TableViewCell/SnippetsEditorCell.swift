...
final class SnippetsEditorCell: NSTextFieldCell {
    ...
    override func draw(withFrame cellFrame: NSRect, in controlView: NSView) {
        ...
        case .folder:
            ...
            let imageName = isHighlighted ? "snippets_icon_folder_white" : "snippets_icon_folder_blue"
            let drawImage = NSImage(named: imageName) ?? NSImage(size: .zero)
        ...
        textColor = (!isItemEnabled) ? .lightGray : (isHighlighted) ? .white : NSColor(named: "title") ?? .labelColor
    ...
    override func select(withFrame aRect: NSRect, in controlView: NSView, editor textObj: NSText, delegate anObject: Any?, start selStart: Int, length selLength: Int) {
        ...
        textColor = NSColor(named: "title") ?? .labelColor
